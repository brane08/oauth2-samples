...
public class JdbcOAuth2AuthorizationService implements OAuth2AuthorizationService {
    ...
    public void save(OAuth2Authorization authorization) {
        ...
        if (this.authorizationRepository.existsById(authorization.getId())) {
            this.authorizationRepository.save(toEntity(authorization));
        } else {
            this.aggregateTemplate.insert(toEntity(authorization));
        }
    }
    ...
    private String writeMap(Map<String, Object> metadata) {
        try {
            return this.securityObjectMapper.writeValueAsString(metadata);
        } catch (Exception ex) {
            throw new IllegalArgumentException(ex.getMessage(), ex);
        }
    }
